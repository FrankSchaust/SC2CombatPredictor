...
def main():
    ...
    tensorboard_dir = os.path.join(REPO_DIR, 'tensorboard_logs', 'All_Conv', 'AdamOpt', 'LearningRate_'+str(lr)+"""  '_Repetitions_'+str(a)+'_'+str(b)+"""'_SampleSize_'+str(cap)+'_'+str(now.year)+'-'+str(now.month)+'-'+str(now.day)+'-'+str(now.hour)+'-'+str(now.minute))
