...
def run_inception(replay_parsed_files, learning_rate, epochs, batches, batch_size, capped_batch, depth, num_classes):
    ...
    with tf.Session() as sess:
        summary_writer = tf.summary.FileWriter('./tensorboard_logs/inception/', sess.graph)
        # initialise the variables
        sess.run(init_op)
        # setup recording variables
        # add a summary to store the accuracy
        cap = 200
        close_matchups = filter_close_matchups(replay_parsed_files)
        ...
        for epoch in range(epochs):
            ...
            for i in range(batches):
                batch_x, batch_y, li = load_batch(replay_parsed_files, train_indices=train_file_indices, capped_batch=batch_size, run=i, lastindex=li, train=True)
            ...
            if len(test_file_indices) < 30: 
                batch_x, batch_y, lis = load_batch(replay_parsed_files, test_indices=test_file_indices, capped_batch=len(test_file_indices), run=1, lastindex=lis,)
                _, c = sess.run([optimiser, cross_entropy], feed_dict={x: batch_x, y: batch_y})
                ...
                for i in range(batches):
                    batch_x, batch_y, lis = load_batch(replay_parsed_files, test_indices=test_file_indices, capped_batch=batch_size, run=i, lastindex=lis,)
                    _, c = sess.run([optimiser, cross_entropy], feed_dict={x: batch_x, y: batch_y})
                    test_acc = sess.run(accuracy, feed_dict={x: batch_x, y: batch_y})
                    #sys.stdout.write("\r[%-20s] %.2f%% --- Batch %d from %d" % ('='*int(((i+1)/total_batch)*20), ((i+1)/total_batch)*100, i+1, total_batch))
                    #sys.stdout.flush()
